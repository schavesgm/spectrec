...
def retrieve_peak_class(name: str):
    """ Return a peak class from the registered set. """
    assert name in __registered_peaks, \
        f'{name =} is not a registered peak: {__registered_peaks.keys()}'
...
def retrieve_kernel_class(name: str):
    """ Return a kernel class from the registered set. """
    assert name in __registered_kernels, \
        f'{name =} is not a registered kernel: {__registered_kernels.keys()}'
